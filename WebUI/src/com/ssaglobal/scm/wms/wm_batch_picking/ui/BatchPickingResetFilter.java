...
public class BatchPickingResetFilter extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	private String CONTEXT_FILTER = "HeaderDetailFilter";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(BatchPickingResetFilter.class);
		...
		else
		{
			...
			if(WSDefaultsUtil.isOwnerLocked(state)){
				...
				if(lockedOwners != null && lockedOwners.size() > 0){
					...
					query = "DPE('SQL','(@[wm_wave.WAVEKEY] IN (SELECT WAVEKEY FROM WAVEDETAIL, ORDERS WHERE WAVEDETAIL.ORDERKEY = ORDERS.ORDERKEY AND ("+lockedOwnerClause+")))')";
