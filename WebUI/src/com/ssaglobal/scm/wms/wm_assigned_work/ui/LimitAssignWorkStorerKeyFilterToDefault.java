...
public class LimitAssignWorkStorerKeyFilterToDefault extends FormExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitAssignWorkStorerKeyFilterToDefault.class);
...
//			throw new UserException(errorMsg,new Object[0]);
		...
		for(int i = 0; i < lockedOwners.size(); i++){
			if(i == 0)
				lockedOwnerClause += " wm_pickdetail.STORERKEY = '" + lockedOwners.get(i) + "' ";
			else
				lockedOwnerClause += " OR wm_pickdetail.STORERKEY = '" + lockedOwners.get(i) + "' ";
		...
		try {
			focus.filterInPlace(new Query("wm_pickdetail",lockedOwnerClause,""));
