...
public class RemoveSpaces extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
    ...
    protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
    	...
    	//_log.debug("LOG_SYSTEM_OUT","[RemoveSpaces]:1",100L);
    	
    	DataBean focus = form.getFocus();
    	if (!focus.isTempBio()){
    		BioBean newBio = (BioBean) focus;
    		
    		for(Iterator i = widgets.iterator(); i.hasNext();){

    			String widget = (String) i.next();
    			
    			//_log.debug("LOG_SYSTEM_OUT","[RemoveSpaces]:2:widget:"+widget+".",100L);

    			String widgetValue = (String)newBio.get(widget);
    			
    			//_log.debug("LOG_SYSTEM_OUT","[RemoveSpaces]:3:widget val:"+widgetValue+".",100L);
    			
    			if (widgetValue != null && widgetValue.length() > 0 && widgetValue.matches("\\s*")){
    				focus.setValue(widget,"");
    				//_log.debug("LOG_SYSTEM_OUT","[RemoveSpaces]:4:changed widget:"+widget,100L);
    			}
    		}
    	}
       return RET_CONTINUE;
