...
public class ValidatePOBeforeReopenAction extends ListSelector {
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ValidatePOBeforeReopenAction.class);
	...
	private void callActionFromNormalForm(DataBean headerFocus)throws EpiException {
		if (headerFocus instanceof BioBean){
				...
				_log.debug("LOG_SYSTEM_OUT","\n POKEY in form:"+POKey+"\n\n",100L);
				if (headerFocus.getValue("STATUS").toString().equalsIgnoreCase("15")){
					String[] ErrorParem = new String[1];
					ErrorParem[0]= POKey;
					UserException UsrExcp = new UserException("WMEXP_VERIFYCLOSE1", ErrorParem);
		 	   		throw UsrExcp;
