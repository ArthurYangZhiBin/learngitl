...
public class CheckPackPreRender extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
		...
		try {
			...
			RuntimeFormWidgetInterface rate = form.getFormWidgetByName("RATE");
			Object rateValue = SessionUtil.getInteractionSessionAttribute(CHECK_RATE, state);
			if (rateValue != null) {
				rate.setDisplayValue(rateValue.toString());
				SessionUtil.setInteractionSessionAttribute(CHECK_RATE, null, state);
			} else {
				rate.setDisplayValue("");
			}
