...
public class AdjustmentHeaderChargeInfoBillCheckAction extends ActionExtensionBase {
	/******************************************************************
	 * Programmer     :        Krishna Kuchipudi
	 * Created        :       18 May 2010
	 * Purpose        :        Updates bill check information
	 *                           - WM 9 3PL Enhancements - Misc Charges Enhancement
	 *******************************************************************
	 * Modification History
	 * SRG 12/15/2010: Instead of making direct updates use OA framework to execute update statements
	 ******************************************************************/
	protected static ILoggerCategory _log = LoggerFactory.getInstance(AdjustmentHeaderChargeInfoBillCheckAction.class);
	...
	protected int execute(ActionContext context, ActionResult result) throws UserException, EpiDataException, EpiException{
		...
		if(form1.isListForm()){
			...
			{				
				...
				String mySql = "UPDATE BILL_CHARGEINSTRUCT SET BILL_CHECK = 1, BILL_CHECK_UID = '"+userLoginId+"', BILL_CHECK_DT ='"+ dateVal+"' WHERE TYPE = 'AJ' AND TRAN_KEY1 = '"+ADJUSTMENTKEY+"' AND TRAN_KEY2 = 0 ";
				try
				{
