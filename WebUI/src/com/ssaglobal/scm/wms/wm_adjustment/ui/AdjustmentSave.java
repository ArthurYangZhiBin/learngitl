...
public class AdjustmentSave extends SaveAction{
    protected static ILoggerCategory _log = LoggerFactory.getInstance(AdjustmentSave.class);

    public AdjustmentSave() { 
	...
	protected int execute(ActionContext context, ActionResult result) throws UserException {	
		...
		if (headerFocus.isTempBio()) {//it is for insert header
			...
			if(detailFocus.isTempBio()){
				...
				String adjustmentKey = (String)headerForm.getFormWidgetByName("ADJUSTMENTKEY").getValue();
				String adjustmentLineNumber = (String)detailForm.getFormWidgetByName("ADJUSTMENTLINENUMBER").getValue();
				...
				if(serialFocus!=null){
					...
					for(int idx = 0; idx < serialTmpBioList.size(); idx++){
						...
						if(qty!=null && qty.doubleValue() != 0.00){
							QBEBioBean newSerial = populateSerial(uowb, serialTmpBio);
				...
				//buildAdjustmentDetailSerials(uowb, adjustmentKey, adjustmentLineNumber);
				//jp End
				
				headerBioBean.addBioCollectionLink("ADJUSTMENTDETAIL", (QBEBioBean)detailFocus);
			}
		} else {
			RuntimeFormInterface detailTab = state.getRuntimeForm(detailForm.getSubSlot("wm_adjustmentdetail_toggle_slot"), "Detail");
			DataBean detailFocus = detailTab.getFocus();
			headerBioBean = (BioBean)headerFocus;
			if (detailFocus.isTempBio()) {
				QBEBioBean qbe = (QBEBioBean)detailFocus;		
				qbe.set("ADJUSTMENTKEY", headerBioBean.get("ADJUSTMENTKEY").toString());
			    uowb.getNewBio(qbe);
			} 		    
		}
		uowb.saveUOW(true);
