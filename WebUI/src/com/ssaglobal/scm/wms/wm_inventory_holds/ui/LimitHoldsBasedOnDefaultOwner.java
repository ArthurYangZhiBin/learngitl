...
import com.epiphany.shr.metadata.objects.generated.np.DataSourceFactory;
...
public class LimitHoldsBasedOnDefaultOwner extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitHoldsBasedOnDefaultOwner.class);
...
//			throw new UserException(errorMsg,new Object[0]);
		...
		for(int i = 0; i < lockedOwners.size(); i++){
			if(i == 0)
				lockedOwnerClause += " LOTXLOCXID.STORERKEY = \\'" + lockedOwners.get(i) + "\\' ";
			else
				lockedOwnerClause += " OR LOTXLOCXID.STORERKEY = \\'" + lockedOwners.get(i) + "\\' ";
		...
		try {
			...
			focus.filterInPlace(new Query("wm_inventoryhold_bio","DPE('SQL','(@[wm_inventoryhold_bio.LOT] IN (SELECT LOT FROM LOTXLOCXID WHERE ("+lockedOwnerClause+"))) OR (@[wm_inventoryhold_bio.LOC] IN (SELECT LOC FROM LOTXLOCXID WHERE ("+lockedOwnerClause+"))) OR (@[wm_inventoryhold_bio.ID] IN (SELECT ID FROM LOTXLOCXID WHERE ("+lockedOwnerClause+")))')",""));
