...
public class HoldsByIdSaveValidation extends com.epiphany.shr.ui.action.ActionExtensionBase {

	protected static ILoggerCategory _log = LoggerFactory.getInstance(HoldsByIdSaveValidation.class);
	
	private static String BIOATTRIBUTE_ID = "ID";
    ...
    */
    ...
    private void validateDataEntry(String holdCode, ArrayList selected) throws UserException{
    	...
    	if(selected==null || selected.size()==0){
    		throw new UserException("WMEXP_NOT_SELECTED_ITEMS", new String[]{"LPNs"});
    	}
    	_log.debug("LOG_SYSTEM_OUT","[HoldsByIdSaveValidation]Number of selected items:" +selected.size(),100L);
    ...
    private void validateLots(UnitOfWorkBean uowb, ArrayList selected , String holdCode, String comment)throws UserException, EpiDataException{
    	for(int idx = 0; idx < selected.size(); idx++){
    		Bio lpnBio = (Bio)selected.get(idx);
    		if(lpnBio==null){
    			_log.debug("LOG_SYSTEM_OUT","[HoldsByIdSaveValidation]lpnBio is null",100L);
    			continue;
    		}
    		
    		
    		String id = (String)lpnBio.get(BIOATTRIBUTE_ID);
    		
    		_log.debug("LOG_SYSTEM_OUT","[HoldsByLotSaveValidation]LPN:"+id,100L);
    		InventoryHoldHelper helper = new InventoryHoldHelper();
    		String hold = helper.getInventoryHoldHold(uowb, BIOATTRIBUTE_ID, id, holdCode);
    		if(hold==null ||hold.equalsIgnoreCase("0")){
    			hold="1";
    			
    			//helper.runInventoryHoldSP(null, null, null, id,holdCode, hold );
    			//Krishna Kuchipudi - Aug-18-2010 - 3PL - Food Enhancements - Starts
				helper.runInventoryHoldSP(null, null, null, id, holdCode, hold, comment );
				//Krishna Kuchipudi - Aug-18-2010 - 3PL - Food Enhancements - Ends
    		}
