...
public class GetCompleteFlag extends com.epiphany.shr.ui.action.ActionExtensionBase {


	protected static ILoggerCategory _log = LoggerFactory.getInstance(GetCompleteFlag.class);
    ...
    */
	protected int execute( ActionContext context, ActionResult result ) throws EpiException {
		...
		_log.debug("LOG_EXTENSION", "jpdebug:GetCompleteFlag:START", 100L);
		
		StateInterface state  = context.getState();
		HttpSession session = state.getRequest().getSession();
		Boolean completeFlag = (Boolean)session.getAttribute(SetCompleteFlag.CLEAR_PHYSICAL_COMPLETE);
		
		if (completeFlag!=null && completeFlag.booleanValue()==true){
			context.setNavigation("closeModalDialog81"); //Formslot
			session.removeAttribute(SetCompleteFlag.CLEAR_PHYSICAL_COMPLETE);
		}else{
			context.setNavigation("closeModalDialog183"); //Modal
		}
		return RET_CONTINUE;
	}
    ...
    */
    protected int execute(ModalActionContext ctx, ActionResult args) throws EpiException {
    	...
    	_log.debug("LOG_EXTENSION", "jpdebug:MODAL:GetCompleteFlag:START", 100L);
