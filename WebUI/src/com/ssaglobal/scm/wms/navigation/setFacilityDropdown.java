...
public class setFacilityDropdown extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
       ...
       try {
    	   RuntimeFormWidgetInterface formWidgetByName = form.getFormWidgetByName("Facility");
			if (formWidgetByName.getAttributeType() == -1) {
				formWidgetByName.setValue(getFacility(context.getState()));
			}
    ...
    throws EpiException
{
	...
	if (session.getAttribute(SetIntoHttpSessionAction.DB_CONNECTION) == null){
		...
		facilityName = session.getAttribute(SetIntoHttpSessionAction.DB_CONNECTION).toString();
	}
    return facilityName;
}
}
