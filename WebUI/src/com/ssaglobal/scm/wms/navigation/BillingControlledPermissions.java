...
public class BillingControlledPermissions extends com.epiphany.shr.ui.view.customization.MenuExtensionBase {
	...
	protected int execute(StateInterface state, RuntimeMenuInterface menu) {
		...
		else menu.setProperty(RuntimeMenuInterface.PROP_HIDDEN, false);
		return RET_CONTINUE;
	}
	...
	protected int execute(StateInterface state, RuntimeMenuItemInterface menuItem) {
		...
		else menuItem.setProperty(RuntimeMenuItemInterface.PROP_HIDDEN, false);
		return RET_CONTINUE;
	}

	boolean toHide(StateInterface state) {
		UnitOfWorkBean uowb = state.getDefaultUnitOfWork();
		String qryStmt = "wm_system_settings.CONFIGKEY='INFORWMS_BILLING'";
