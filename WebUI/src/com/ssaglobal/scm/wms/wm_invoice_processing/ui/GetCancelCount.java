...
public class GetCancelCount  extends FormWidgetExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(GetCancelCount.class);

	public GetCancelCount()
	{
	}

	protected int execute(StateInterface state, RuntimeFormWidgetInterface widget) throws EpiException
	{
	  	 _log.debug("LOG_DEBUG_EXTENSION_INV_PROC","Executing GetCancelCount",100L);
		try
		{
			...
			try
			{
				...
				{
					args[0] = (String)state.getServiceManager().getUserContext().get("CANCELCOUNT").toString();
				
				//args[0] = state.getServiceManager().getUserContext().get("CANCELCOUNT").toString();
				}
			...
			strMsg = getTextMessage("WMTXT_TOTAL_CANCELLED", args, locale);
			...
			state.getServiceManager().getUserContext().remove("CANCELCOUNT");
		} catch (Exception e)
		{
			e.printStackTrace();
			state.getServiceManager().getUserContext().remove("CANCELCOUNT");
		...
		_log.debug("LOG_DEBUG_EXTENSION_INV_PROC","Exiting GetCancelCount",100L);
		return 0;
	}
}