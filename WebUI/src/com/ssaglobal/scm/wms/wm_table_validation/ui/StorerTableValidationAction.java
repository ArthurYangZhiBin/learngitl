...
public class StorerTableValidationAction extends ActionExtensionBase{
	protected int execute(ActionContext context, ActionResult result)throws EpiException{
		...
		if(sourceWidgetValue!=null){
			...
			try
			{
				...
				if(typeValue == null || "".equalsIgnoreCase(typeValue.trim())){
		...
		// Result Found
		QBEBioBean qbe = (QBEBioBean)sourceWidget.getForm().getFocus();
		if(typeValue != null && !"".equalsIgnoreCase(typeValue)){
			try{
				qbe.set(ATTRIBUTE, sourceWidgetValue);
			}catch(NullPointerException e){
				//Data validation on Storer widget not in a bio.
				sourceWidget.setDisplayValue(sourceWidgetValue);
			}catch(RuntimeException e){
				qbe.set(sourceWidget.getAttribute(), sourceWidgetValue);
			}
		}
