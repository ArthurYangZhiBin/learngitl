...
public class PackTableValidation extends com.epiphany.shr.data.bio.extensions.BioExtensionBase
{
	private static String BIO = "wm_pack";

	private static String ATTRIBUTE = "PACKKEY";

	private static String ERROR_MESSAGE = "Invalid PACKKEY- ";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(PackTableValidation.class);

	public PackTableValidation()
	{
		_log.info("EXP_1", "PackTableValidation has been instantiated...", SuggestedCategory.NONE);
	...
	protected int bioBeforeUpdate(EpnyServiceContext context, Bio bio) throws EpiException
	{
		...
		return validatePack(context, bio);
	}
	...
	private int validatePack(EpnyServiceContext context, Bio bio) throws EpiException
	{
		_log.info("EXP_1", "Start of PackTableValidation", SuggestedCategory.NONE);
		...
		// Query Bio to see if Pack exists
		...
		for (Iterator it = attributes.entrySet().iterator(); it.hasNext();)
		{
			...
			{
				e.printStackTrace();
				throw new EpiException("EXP_VALIDATE_PACK_ERROR", "Error preparing search query");
			}

			// If BioCollection size equals 0, return RET_CANCEL
			if (results.size() == 0)
			{
				_log.debug("LOG_SYSTEM_OUT","////Packkey Invalid",100L);
				throw new FormException(ERROR_MESSAGE + attributeName + ": " + attributeValue, new Object[] {});

			}
			else
			{
				_log.debug("LOG_SYSTEM_OUT","////Packkey Valid " + attributeName + ": " + attributeValue,100L);
			}
