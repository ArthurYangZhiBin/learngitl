 ...
 */
public class ItemStorePackInBioCCF extends com.epiphany.shr.ui.action.ccf.SendAllWidgetsValuesExtensionBase
{
	 ...
	 */
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ItemStorePackInBioCCF.class);

	protected int execute(RuntimeFormInterface form, RuntimeFormWidgetInterface formWidget, HashMap params)
			throws EpiException
	{
		_log.debug("LOG_SYSTEM_OUT","\n\n\n\n\n Setting Pack into Bio",100L);
		String pack = params.get("fieldValue").toString();
		_log.debug("LOG_SYSTEM_OUT","PACK : " + pack,100L);
		...
		for (Iterator it = allWidgets.entrySet().iterator(); it.hasNext();)
		{
			...
			try
			{
				_log.debug("LOG_SYSTEM_OUT","*& " + key.getName(),100L);
			} catch (NullPointerException e)
			{
				_log.debug("LOG_SYSTEM_OUT","Exception Caught" + e.getMessage(),100L);
			}
		}
		...
		else if (formFocus instanceof QBEBioBean)
		{
			_log.debug("LOG_SYSTEM_OUT","QBEBioBean",100L);
			formFocus = (QBEBioBean) formFocus;
		}
		else
		{
			_log.debug("LOG_SYSTEM_OUT","Not BioBean or QBEBioBean",100L);
		}
		formFocus.setValue("PACKKEY", pack);
		formFocus.setValue("PACKTEMP", pack);
		setValue(form.getFormWidgetByName("PACKKEY"), pack);
		setValue(form.getFormWidgetByName("PACKTEMP"), pack);
		
		DataBean readFocus = form.getFocus();
		_log.debug("LOG_SYSTEM_OUT","Packkey: " + readFocus.getValue("PACKKEY"),100L);
		_log.debug("LOG_SYSTEM_OUT","Packtemp: " + readFocus.getValue("PACKTEMP"),100L);
