 ...
 */
public class ItemRetrieveAssociatedPackCCF extends com.epiphany.shr.ui.action.ccf.SendAllWidgetsValuesExtensionBase
{
	...
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ItemRetrieveAssociatedPackCCF.class);
			...
			throws EpiException
	{
		...
		_log.debug("LOG_SYSTEM_OUT","=-=-= ItemRetrieveAssociatedPack start",100L);
		try
		{
			String packAttribute = getParameterString("PACKATTR");
			String itemValue = params.get("fieldValue").toString();
			_log.debug("LOG_SYSTEM_OUT","$%^ SKU : " + itemValue,100L);
			...
			if (results.size() == 1)
			{
				...
				_log.debug("LOG_SYSTEM_OUT","\n!@# Setting " + packAttribute + " to " + resultBio.get(PACKKEY),100L);
				setValue(form.getFormWidgetByName(packAttribute), resultBio.get(PACKKEY).toString());

			}
			else
			{
				//SKU does not exist
				_log.debug("LOG_SYSTEM_OUT","Item(SKU) does not exist",100L);
				return RET_CANCEL;
			}
