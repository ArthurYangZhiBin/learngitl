...
public class IndirectActivityRefreshUAKPreRender extends com.epiphany.shr.ui.view.customization.FormExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(IndirectActivityRefreshUAKPreRender.class);
		...
		try
		{
			...
			for (int i = 0; i < rows.length; i++)
			{
				...
				else
				{
					...
					sc.addCondition(AttributeConstants.ID, uidWidget.getDisplayValue(),
				...
				//Set UAK
				RuntimeFormWidgetInterface uakWidget = rows[i].getFormWidgetByName("USERATTENDANCEKEY");
				RuntimeFormWidgetInterface newUakWidget = rows[i].getFormWidgetByName("NEWUAK");
				newUakWidget.setDisplayValue(uakWidget.getDisplayValue());
