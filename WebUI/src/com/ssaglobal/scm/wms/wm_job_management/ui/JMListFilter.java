...
public class JMListFilter extends com.epiphany.shr.ui.view.customization.FormExtensionBase
{
	 ...
	 */
	protected static ILoggerCategory _log = LoggerFactory.getInstance(JMListFilter.class);
	...
	protected int preRenderListForm(UIRenderContext context, RuntimeListFormInterface form) throws EpiException
	{
		...
		try
		{
			...
			_log.debug("LOG_SYSTEM_OUT","\n\t" + "Size Before " + listFocus.size() + "\n",100L);
			if (form.getName().equalsIgnoreCase("wm_job_management_detail_NFA_view")){
				filterQry = new Query("wm_jm_orderdetail", "wm_jm_orderdetail.OPENQTY > wm_jm_orderdetail.QTYALLOCATED", null);
			}else{
				filterQry = new Query("wm_jm_orderdetail", "wm_jm_orderdetail.OPENQTY > wm_jm_orderdetail.QTYPREALLOCATED", null);				
			}

			BioCollection newFocus = listFocus.filter(filterQry);

			_log.debug("LOG_SYSTEM_OUT","\n\t" + "Size After " + newFocus.size() + "\n",100L);
