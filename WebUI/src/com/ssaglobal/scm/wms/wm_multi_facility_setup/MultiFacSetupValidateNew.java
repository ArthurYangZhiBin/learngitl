...
public class MultiFacSetupValidateNew extends ActionExtensionBase
{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(MultiFacSetupValidateNew.class);    

    protected int execute(ActionContext context, ActionResult result)
        throws UserException
    {
    	_log.debug("LOG_DEBUG_EXTENSION_MULTIFACSETUPPROP","Executing MultiFacSetupValidateNew",100L);    	
		...
		if(((Integer)facilityRecord.get("LEVELNUM")).intValue() == -1){
			_log.debug("LOG_ERROR_EXTENSION_MULTIFACSETUPPROP","Selected Facility Is At Warehouse Level...",100L);        	
			String args[] = new String[0]; 
			String errorMsg = getTextMessage("WMEXP_MULTI_FAC_BAL_SELECTED_WAREHOUSE",args,state.getLocale());
			throw new UserException(errorMsg,new Object[0]);
		}
		
        _log.debug("LOG_DEBUG_EXTENSION_MULTIFACSETUPPROP","Exiting MultiFacSetupValidateNew",100L);
        return RET_CONTINUE;
    }
}
