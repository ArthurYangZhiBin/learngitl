...
public class TaskDetailUOMCompAttrDom extends AttributeDomainExtensionBase{
	...
	public int execute(DropdownContentsContext context, List values, List labels) throws EpiDataException{
		...
		//Set query
		String query = "SELECT 2,PACKUOM1 AS PACKUOM FROM PACK WHERE PACKUOM1 <> ' ' AND PACKKEY = '"+pack+"' UNION " +
				"SELECT 3, PACKUOM2 AS PACKUOM FROM PACK WHERE PACKUOM2 <> ' ' AND PACKKEY = '"+pack+"' UNION " +
				"SELECT 6, PACKUOM3 AS PACKUOM FROM PACK WHERE PACKUOM3 <> ' ' AND PACKKEY = '"+pack+"' UNION " +
				"SELECT 1, PACKUOM4 AS PACKUOM FROM PACK WHERE PACKUOM4 <> ' ' AND PACKKEY = '"+pack+"' UNION " +
				"SELECT 4, PACKUOM8 AS PACKUOM FROM PACK WHERE PACKUOM8 <> ' ' AND PACKKEY = '"+pack+"' UNION " +
				"SELECT 5, PACKUOM9 AS PACKUOM FROM PACK WHERE PACKUOM9 <> ' ' AND PACKKEY = '"+pack+"'";
