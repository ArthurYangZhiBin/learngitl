...
public class ShipmentOrderPopupError extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(ModalUIRenderContext context, RuntimeNormalFormInterface form) throws EpiException {
		...
		try {
			...
			ArrayList<String> suspendedOrders = (ArrayList<String>) SessionUtil.getInteractionSessionAttribute(	ShipmentOrderPreSave.SO_LIST_ERROR_MSG,
																												state);
			if (suspendedOrders != null) {
				String textMessage = getTextMessage("WMEXP_SO_UNABLE_SAVE_SUSPENDED",
													new Object[] { suspendedOrders.toString().substring(1,
																										suspendedOrders.toString().length() - 1) },
													state.getLocale());
				form.getFormWidgetByName("ERROR").getLabel(RuntimeFormWidgetInterface.LABEL_VALUE, state.getLocale());
				form.getFormWidgetByName("ERROR").setLabel(RuntimeFormWidgetInterface.LABEL_VALUE, textMessage);
			}
