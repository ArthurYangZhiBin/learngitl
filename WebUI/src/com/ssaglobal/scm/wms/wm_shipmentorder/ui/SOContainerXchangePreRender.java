 ...
 * Created			:       01/Dec/2009
 * Purpose	        :       Shipment Order Container Exchange prerender Extension to 
...
public class SOContainerXchangePreRender extends com.epiphany.shr.ui.view.customization.FormExtensionBase {

    /** Added for 3PL Enhancements
        	   ...
        	   if(detailFocus instanceof QBEBioBean)
         	   {
        		   if(source.getName().equalsIgnoreCase("NEW")){
	        		   ...
	        		   qbe.setValue("EXTERNORDERKEY", headerFocus.getValue("EXTERNORDERKEY"));
	        		   detailFocus.setValue("ORDERKEY", headerFocus.getValue("ORDERKEY"));
	        		   if(detailListFocus instanceof QBEBioBean)
	        		   {
	        			   qbe.setValue("ORDERXLINENUMBER", "00001");
	        			   return RET_CONTINUE;
	        		   }
	        		   ...
	        		   try {
	    					if(bioFocus.size()!=0){						
	    						...
	    						for(int i = 0; i < bioFocus.size(); i++){				
	    							Object bioFieldValueObj = bioFocus.elementAt(i).get("ORDERXLINENUMBER");
	    							try {
	    			...
	    			if(size>99999){
	    				form.getFormWidgetByName("ORDERXLINENUMBER").setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.TRUE);
	    				throw new FormException("WMEXP_VALUE_GEN_FAIL", null);
	    			}else{
	    				key = template.format(size);
	    			}
	    			// Store value
	    			qbe.setValue("ORDERXLINENUMBER", key);
        		   }    			
         	   }
        	   else
        	   {
