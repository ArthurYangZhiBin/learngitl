...
public class ShipmentOrderDetailPickAction extends
		com.epiphany.shr.ui.action.ActionExtensionBase {

	/** The Constant SOD_PICK. */
	private static final String SOD_PICK = "SOD_PICK";

	/** The log. */
	protected static ILoggerCategory log = LoggerFactory
			.getInstance(ShipmentOrderDetailPickAction.class);
	...
	private void beforePick(ActionContext context) throws EpiDataException {
		...
		if (currentFocus.isBioCollection()) {
			...
			SessionUtil.setInteractionSessionAttribute(SOD_PICK, orderLines,
					state);
			SessionUtil.setInteractionSessionAttribute(
					ShipmentOrderMessagePrerender.SO_MESSAGE,
					"WMTXT_SOD_PICK_LIST", state);
			SessionUtil.setInteractionSessionAttribute(
	...
	private void clearSessionVariables(ActionContext context) {
		StateInterface state = context.getState();
		SessionUtil.setInteractionSessionAttribute(SOD_PICK, null, state);
		SessionUtil.setInteractionSessionAttribute(
	 ...
	 *             the user exception
	 */
	private void pick(ActionContext context) throws EpiDataException,
			UserException {
		StateInterface state = context.getState();
	 ...
	 *             the epi data exception
	 */
	private boolean shouldPickOrderDetail(BioBean currentFocus)
			throws EpiDataException {
