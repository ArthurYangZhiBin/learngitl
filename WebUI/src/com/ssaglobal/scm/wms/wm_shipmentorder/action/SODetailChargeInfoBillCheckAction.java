...
public class SODetailChargeInfoBillCheckAction extends ActionExtensionBase {
	protected static ILoggerCategory _log = LoggerFactory.getInstance(SOChargeInfoBillCheckAction.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException, EpiDataException, EpiException{
		...
		if(form1.isListForm()){
			...
			{				
				...
				OrderKey = bio.get("TRAN_KEY1").toString();
				OrderLineNumber = bio.get("TRAN_KEY2").toString();
				String mySql ="UPDATE BILL_CHARGEINSTRUCT SET BILL_CHECK = 1, BILL_CHECK_UID = '"+userLoginId+"', BILL_CHECK_DT ='"+ dateVal+"' WHERE TYPE = 'SO' AND TRAN_KEY1 = '"+OrderKey+"' AND TRAN_KEY2 ='"+OrderLineNumber+"'";
				try
				{
