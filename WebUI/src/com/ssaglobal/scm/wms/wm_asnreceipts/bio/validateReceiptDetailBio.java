...
public class validateReceiptDetailBio extends com.epiphany.shr.data.bio.extensions.BioExtensionBase {
	protected static ILoggerCategory _log = LoggerFactory.getInstance(validateReceiptDetailBio.class);
	/**
	...
	protected int bioBeforeDelete( EpnyServiceContext context, Bio bio ) throws EpiException {
		...
		BigDecimal qtyReceived = (BigDecimal)bio.get("QTYRECEIVED");
		BigDecimal qtyExpected = (BigDecimal)bio.get("QTYEXPECTED");
		String[] process = new String[1];
		process[0] = receiptLinenumber;
		if (qtyReceived.intValue() > 0 ){
			throw new UserException("WMEXP_DOCUMENT_NODELETE_1", process);
		}
		if ((type.equalsIgnoreCase("3") && status.equalsIgnoreCase("2") && 
				qtyReceived.intValue() == 0) && qtyExpected.intValue() > 0){
			throw new UserException("WMEXP_DOCUMENT_NODELETE_2", process);
		}
	 ...
	 */
        protected int bioFinalBeforeDelete( EpnyServiceContext context, Bio bio ) throws EpiException {
    		...
    		String type = bio.get("TYPE").toString();
    		String status = bio.get("STATUS").toString();
    		String ASNKey = bio.get("RECEIPTKEY").toString();
    		String[] process = new String[1];
    		process[0] = ASNKey;
    		BioCollection ReceiptDetailBiocollection = (BioCollection)bio.get("RECEIPTDETAILS");
    		int j = 0;
            for (j=0; j<ReceiptDetailBiocollection.size(); j++){
            	
            	Bio ReceiptDetailLine = (Bio)ReceiptDetailBiocollection.elementAt(j);
            	BigDecimal qtyReceived = (BigDecimal)ReceiptDetailLine.get("QTYRECEIVED");
            	BigDecimal qtyExpected = (BigDecimal)ReceiptDetailLine.get("QTYEXPECTED");
            	if (qtyReceived.intValue() > 0 ){
            		throw new UserException("WMEXP_DOCUMENT_NODELETE_1", process);
            	}
            	if ((type.equalsIgnoreCase("3") && status.equalsIgnoreCase("2") && 
