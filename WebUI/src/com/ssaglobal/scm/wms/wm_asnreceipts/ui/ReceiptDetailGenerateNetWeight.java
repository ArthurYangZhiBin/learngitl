...
public class ReceiptDetailGenerateNetWeight extends ActionExtensionBase{
	/******************************************************************
     * Programmer:        	   Krishna Kuchipudi
     * Created        :        20/01/2010
     * Purpose        :       3PL Enhancements - Catch weights tracking -When user enters grosswt value (in wms_ASN_Line_Detail_view form)
     * the netwt comes as 'GrossWt-TareWt'.
     *******************************************************************
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException{
		...
		float fltQtyReceived = Float.parseFloat(strQtyReceived);
...
//			RuntimeFormInterface normalHeaderForm = state.getRuntimeForm(headerTbgrpSlot, TAB0);			
			...
			Object o1 = currentForm.getFormWidgetByName(grosswgt1).getDisplayValue();
			dblGrossWgt = new Double(currentForm.getFormWidgetByName(grosswgt1).getDisplayValue()).doubleValue();
			dblTareWgt = new Double(currentForm.getFormWidgetByName(tarewgt1).getDisplayValue()).doubleValue();
			dblNetWgt = dblGrossWgt-dblTareWgt;
