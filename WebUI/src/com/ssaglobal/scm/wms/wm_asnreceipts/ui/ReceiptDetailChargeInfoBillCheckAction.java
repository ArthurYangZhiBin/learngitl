...
public class ReceiptDetailChargeInfoBillCheckAction  extends ActionExtensionBase{
	//Created for 3PL Enhancements - Misc Charges - Krishna
	...
	protected int execute(ActionContext context, ActionResult result) throws UserException, EpiDataException, EpiException{
		...
		if(form1.isListForm()){
			...
			{				
				...
				ReceiptLineNumber = bio.get("TRAN_KEY2").toString();
				String mySql ="UPDATE BILL_CHARGEINSTRUCT SET BILL_CHECK = 1, BILL_CHECK_UID = '"+userLoginId+"', BILL_CHECK_DT ='"+ dateVal+"' WHERE TYPE = 'RC' AND TRAN_KEY1 = '"+ReceiptKey+"' AND TRAN_KEY2 ='"+ReceiptLineNumber+"'";
				try
				{
