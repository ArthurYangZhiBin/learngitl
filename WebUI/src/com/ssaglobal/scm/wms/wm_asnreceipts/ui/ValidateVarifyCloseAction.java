...
public class ValidateVarifyCloseAction extends ListSelector {
	public static String RECORDCOUNT = "recordcount";
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ValidateVarifyCloseAction.class);
	protected int execute( ActionContext context, ActionResult result ) throws EpiException {
		...
		if (headerFocus instanceof BioCollection){
			...
			if(headerListForm.getSelectedItems().size() > 0)
			{
		        String strCount = headerListForm.getSelectedItems().size()+ "";
		        session.setAttribute(RECORDCOUNT,strCount);
			}else{
