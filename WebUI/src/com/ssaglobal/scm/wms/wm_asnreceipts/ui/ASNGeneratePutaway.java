...
public class ASNGeneratePutaway extends ListSelector {
	public static String TASKCOUNT = "taskcount";
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ASNGeneratePutaway.class);
	
	protected int execute( ActionContext context, ActionResult result ) throws EpiException {
		_log.debug("LOG_SYSTEM_OUT","\n\nExecuting Generate Putaway \n\n",100L);
		...
		if (headerFocus instanceof BioCollection){
			//Executes SP name:GeneratePaperPutaway params:RECEIPTKEY
			...
			{
				_log.debug("LOG_SYSTEM_OUT","\n\nExecuting Generate Putaway GeneratePaperPutaway \n\n",100L);
...
//				{
					...
					for(; bioBeanIter.hasNext();){
						...
						actionProperties.setProcedureName("GeneratePaperPutaway");
						try {
							EXEDataObject dataObject = WmsWebuiActionsImpl.doAction(actionProperties);
							String taskCount = dataObject.getAttribValue(new TextData("taskcount")).toString();
							session.setAttribute(TASKCOUNT,taskCount );
							_log.debug("LOG_SYSTEM_OUT","TASKCOUNT = "+ session.getAttribute(TASKCOUNT),100L);
						} catch (Exception e) {
...
//				}
			...
			callActionFromNormalForm(headerFocus, session);
		}
		_log.debug("LOG_SYSTEM_OUT","\n\nExecuting Action10 \n\n",100L);
		return RET_CONTINUE;
	...
	private void callActionFromNormalForm(DataBean headerFocus, HttpSession session)throws EpiException {
