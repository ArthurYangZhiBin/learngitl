...
public class ASNReceiptPrintLabelAndExplode extends ListSelector {

	protected static ILoggerCategory _log = LoggerFactory.getInstance(ASNReceiptPrintLabelAndExplode.class);
	protected int execute(ModalActionContext context, ActionResult result)
			throws EpiException {
		/******************************************************************
	     * Programmer:        	   Krishna Kuchipudi
	     * Created        :        23/12/2010
	     * Purpose        :        Call the relevant EJB for each selected 
		...
		String printerName = " ";
		String copies = " ";
		//Krishna Kuchipudi: Dec-16-2009: ASN Explode related code: Added the below variable
		String isLabelToPrinted = "1";
		String slotNum = "";
		int slot = 9;
			...
			if (slot == 0) {// it is ASN Detail List
				...
				if (detailFocus instanceof BioCollection) { // if list view
					...
					if (selectedItems != null && selectedItems.size() > 0) { // get list of selected records
						...
						//Krishna Kuchipudi: Dec-16-2009: ASN Explode and Print related code: Starts
			                	
			                	System.out.println("\n\nRECEIPTKEY:"+ReceiptKey+"\n\n");
			                	System.out.println("\n\nRECEIPTLINENUMBER:"+ReceiptlineNumber +"\n\n");
			                	WmsWebuiActionsProperties actionProperties1 = new WmsWebuiActionsProperties();
			                	Array parms1 = new Array(); 
			                	parms1.add(new TextData(ReceiptKey));
			                	parms1.add(new TextData(ReceiptlineNumber));
			                	parms1.add(new TextData(labelName));
								parms1.add(new TextData(printerName));
								parms1.add(new TextData(copies));
								parms1.add(new TextData(isLabelToPrinted));
			                	actionProperties1.setProcedureParameters(parms1);
			                	actionProperties1.setProcedureName("NSPEXPLODEASNLINE");
							try {
								WmsWebuiActionsImpl.doAction(actionProperties1);
							} catch (Exception e) {
								e.getMessage();
								UserException UsrExcp = new UserException(e.getMessage(), new Object[]{});
					 	   		throw UsrExcp;
							}
						//Krishna Kuchipudi: Dec-16-2009: ASN Explode and Print related code:  Ends
							//Krishna Kuchipudi: Dec-16-2009: ASN Explode and Print related code: Commented the below code
							/*
								WmsWebuiActionsProperties actionProperties = new WmsWebuiActionsProperties();
								Array parms = new Array();
								parms.add(new TextData(ReceiptKey));
								...
								try {
									...
									UserException UsrExcp = new UserException(e
											.getMessage(), new Object[] {});
									throw UsrExcp;
								}*/ //Krishna Kuchipudi: Dec-16-2009: ASN Explode and Print related code: Above Code Commented
