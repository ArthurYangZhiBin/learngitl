...
import com.ssaglobal.scm.wms.wm_po.ui.populateSellerInfo;
...
public class AutoFillTrailerDescription extends com.epiphany.shr.ui.view.customization.FormExtensionBase {

	protected static ILoggerCategory _log = LoggerFactory.getInstance(AutoFillTrailerDescription.class);
	/**
	...
	throws EpiException {
		...
		try {
			...
			if(context.getState().getRequest().getSession().getAttribute("TRAILERNUMBERAUTO") != null)
			{
				trailerNumber = context.getState().getRequest().getSession().getAttribute("TRAILERNUMBERAUTO").toString();
				form.getFormWidgetByName("TrailerNumber").setDisplayValue(trailerNumber);
				context.getState().getRequest().getSession().setAttribute("TRAILERNUMBERAUTO", null);
			}
			else
			{
				trailerNumber = form.getFormWidgetByName("TrailerNumber").getDisplayValue() == null || form.getFormWidgetByName("TrailerNumber").getDisplayValue().toString().matches("\\s*") ? "" : form.getFormWidgetByName("TrailerNumber").getDisplayValue().toString();
			}
