...
public class ASNReceiptItemLookupQuery extends ActionExtensionBase {

	protected static ILoggerCategory _log = LoggerFactory.getInstance(ASNReceiptItemLookupQuery.class);
	//Specific query for an item lookup that only returns the selected owner 
	protected int execute(ActionContext context, ActionResult result)throws EpiException{

		String qry = "sku.STORERKEY = '";
		String qry1 = "wm_storer.STORERKEY = '";
		StateInterface state = context.getState();
//		String shellSlot1 = getParameter("shellSlot1").toString();
//		String listShellForm = getParameter("listShellForm").toString();
		String value = null;
...
//		_log.debug("LOG_SYSTEM_OUT","FINDING UNCAUGHT EXCEPTION: "+normalHeaderForm.getName(),100L);
		value = state.getCurrentRuntimeForm().getFormWidgetByName("STORERKEY").getDisplayValue();
		_log.debug("LOG_SYSTEM_OUT","FINDING UNCAUGHT EXCEPTION: "+value,100L);
		UnitOfWorkBean uow = state.getDefaultUnitOfWork();
		qry1 = qry1 + value + "' AND wm_storer.TYPE = '1'"; 
		Query CheckOwnerQry = new Query("wm_storer", qry1, null);
		BioCollectionBean OwnerFocus = uow.getBioCollectionBean(CheckOwnerQry);
		if(OwnerFocus.size()<1){
			String[] parameters = new String[1];
			parameters[0] = value;
			throw new FormException("WMEXP_OWNER_VALID", parameters);
		}
		qry = qry + value + "'";
		Query loadBiosQry = new Query("sku", qry, null);
		BioCollectionBean newFocus = uow.getBioCollectionBean(loadBiosQry);
		if(newFocus.size()<1){
			throw new FormException("WMWXP_SKUBYOWNERVALIDATION", new Object[]{});
		}
