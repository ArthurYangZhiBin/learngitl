...
/*********************************************************************************
 * Krishna Kuchipudi: Dec-04-2009: This class is added as an extension for
 * Container Activity Report.
 **********************************************************************************/
...
public class ReceiptContainerActivity extends ListSelector {
	private final String LISTVIEW = "closeModalDialog72";	
	private final String DETAILVIEW = "closeModalDialog71";
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ReceiptContainerActivity.class);
		...
		if (headerFocus instanceof BioCollection){
			...
			if(selectedItems != null && selectedItems.size() > 0)
			{
			   ...
			   //paramsAndValues.put("p_ReceiptTo", receiptEndS);
			    ...
			    paramsAndValues.put("p_DocumentIDStartS",  receiptStartS);
				paramsAndValues.put("p_DocumentIDEndS", receiptEndS);
			    
				String DOC_TYPE = "DP";
				paramsAndValues.put("p_DOC_TYPE", DOC_TYPE);
			    
				String report_url= ReportUtil.retrieveReportURL(state,"CRPT998", paramsAndValues);
...
//			context.setNavigation(DETAILVIEW);
	...
	private void callActionFromNormalForm(ActionContext context,DataBean headerFocus)throws EpiException {
		if (headerFocus instanceof BioBean){
				...
				String DocumentIDStartS = headerFocus.getValue("RECEIPTKEY").toString();
				String DocumentIDEndS = headerFocus.getValue("RECEIPTKEY").toString();
				...
				*/
				...
				paramsAndValues.put("p_RDateFrom",pStartDate);
				paramsAndValues.put("p_RDateTo",pEndDate);
				paramsAndValues.put("p_DocumentIDStartS",  DocumentIDStartS);
				paramsAndValues.put("p_DocumentIDEndS", DocumentIDEndS);
				String DOC_TYPE = "ASN";
				paramsAndValues.put("p_DOC_TYPE", DOC_TYPE);
