...
public class AddLPNPreRender extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(ModalUIRenderContext context, RuntimeNormalFormInterface form) throws EpiException {
		...
		try {
			...
			RuntimeFormWidgetInterface stopWidget = form.getFormWidgetByName("STOP");
			if (StringUtils.isEmpty(stopWidget.getDisplayValue())) {
				stopWidget.setDisplayValue("1");
			}
			for (Iterator it = form.getFormWidgets(); it.hasNext();) {
				((RuntimeFormWidgetInterface) it.next()).setProperty("cursor focus widget", "false");
			}
			// Set cursor to LPN
			RuntimeFormWidgetInterface lpnWidget = form.getFormWidgetByName("LPN");
			lpnWidget.setProperty("cursor focus widget", "true");
