...
public class LoadMaintenanceResetFilter extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	private String CONTEXT_FILTER = "HeaderDetailFilter";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(LoadMaintenanceResetFilter.class);
		...
		else
		{
			...
			if(WSDefaultsUtil.isOwnerLocked(state)){
				...
				if((lockedOwners != null && lockedOwners.size() > 0)
					...
					query = "DPE('SQL','@[wm_loadhdr_auto_pk.LOADID] in ("+innerQuery+")')";
