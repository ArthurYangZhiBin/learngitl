...
public class DisableLoadLPNAction extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form) throws EpiException {
		...
		try {
			...
			if (headerForm != null) {
				...
				String statusVal = BioAttributeUtil.getStringNoNull(focus, "STATUS");
				if (!StringUtils.isEmpty(statusVal)) {
					int status = Integer.parseInt(statusVal);
					if (status >= 7) {
						form.getFormWidgetByName("LOADLPN").setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.TRUE);
					} else {
						form.getFormWidgetByName("LOADLPN").setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.FALSE);
					}
				}
			}
