...
public class ValidatePackKeyOnChange extends ActionExtensionBase
{

	protected static ILoggerCategory _log = LoggerFactory.getInstance(ValidatePackKeyOnChange.class);
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException
	{
		...
		if(displayValue != null){
			String packKey = displayValue.trim().toUpperCase();
//			Validate Storer Key, if present must be present in STORER table and of type 1
			if(packKey.length() > 0){
				try {
					...
					Query loadBiosQry = new Query("wm_pack", "wm_pack.PACKKEY = '"+packKey.toUpperCase()+"'", "");
