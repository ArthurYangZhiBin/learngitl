...
public class ListSelectDisableCellOnValue extends com.epiphany.shr.ui.view.customization.FormExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ListSelectDisableCellOnValue.class);
	...
	protected int modifyListValues(UIRenderContext context, RuntimeListFormInterface form) throws EpiException
	{
		...
		ArrayList values =  (ArrayList) getParameter("VALUES");
		...
		try
		{
			for (int i = 0; i < listRows.length; i++)
			{
				for (int j = 0; j < attributes.size(); j++)
				{
					...
					if (values.contains(statusValue))
					{
						_log.debug("LOG_DEBUG_EXTENSION_ListSelectDisableCellOnValue", "Disabling cell in Column: "
