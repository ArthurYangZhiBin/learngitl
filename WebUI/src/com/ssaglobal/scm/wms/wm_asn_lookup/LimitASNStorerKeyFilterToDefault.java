...
public class LimitASNStorerKeyFilterToDefault extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitASNStorerKeyFilterToDefault.class);
...
//			throw new UserException(errorMsg,new Object[0]);
		...
		for(int i = 0; i < lockedOwners.size(); i++){
			if(i == 0)
				lockedOwnerClause += " receiptdetail.STORERKEY = '" + lockedOwners.get(i) + "' ";
			else
				lockedOwnerClause += " OR receiptdetail.STORERKEY = '" + lockedOwners.get(i) + "' ";
		}
		
		BioCollectionBean focus = (BioCollectionBean)result.getFocus();
		try {
			focus.filterInPlace(new Query("receiptdetail",lockedOwnerClause,""));
		} catch (EpiDataException e) { 
			_log.error("LOG_DEBUG_EXTENSION_LIMITSTORKEY","Exception Occured. Exiting...",100L);
