...
public class OwnerSetLPNNextOnSave extends com.epiphany.shr.data.bio.extensions.BioExtensionBase
{
			...
			throws EpiException
	{
		...
		return testLPNNext(context, bio);
	}
	...
	private int testLPNNext(EpnyServiceContext context, Bio bio)
	{
		Object lpnNext = null;
		try
		{
			lpnNext = bio.get("NEXTLPNNUMBER");
		} catch (EpiDataException e)
		{
			// TODO Auto-generated catch block
			//e.printStackTrace();
			//_log.debug("LOG_SYSTEM_OUT","!@# setting LPN Next to LPN Start",100L);
			setLPNNext(bio);
		}

		if (lpnNext == null)
		{
			//_log.debug("LOG_SYSTEM_OUT","!@# Null, setting LPN Next to LPN Start",100L);
			setLPNNext(bio);
		}
		else if (lpnNext.toString().matches("\\s*"))
		{
			//_log.debug("LOG_SYSTEM_OUT","!@# Blank, setting LPN Next to LPN Start",100L);
			setLPNNext(bio);
		}
		else
		{
			//_log.debug("LOG_SYSTEM_OUT","!@# LPN Next is set - " + lpnNext.toString(),100L);
		}

		return RET_CONTINUE;
	}

	private void setLPNNext(Bio bio)
	{
		try
		{
			bio.set("NEXTLPNNUMBER", bio.get("LPNSTARTNUMBER"));
		} catch (EpiDataException e)
		{
			// TODO Auto-generated catch block
			//_log.debug("LOG_SYSTEM_OUT","!@# Exception caught, again",100L);
			//e.printStackTrace();
		}
	}
