...
public class LocTableValidation extends com.epiphany.shr.data.bio.extensions.BioExtensionBase
{
	private static String BIO = "wm_location";

	private static String ATTRIBUTE = "LOC";

	private static String ERROR_MESSAGE = "LOCATION_VALIDATION";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(LocTableValidation.class);

	public LocTableValidation()
	{
	...
	{
		_log.info("EXP_1", "Start of LocTableValidation", SuggestedCategory.NONE);
		_log.debug("LOG_SYSTEM_OUT","\n\n\n\n!@# LocTableValidation Start !@#",100L);
		...
		for (Iterator it = attributes.entrySet().iterator(); it.hasNext();)
		{
			...
			if (results.size() == 0)
			{
				_log.debug("LOG_SYSTEM_OUT","////Loc Invalid",100L);
				String[] parameters = new String[1];
				parameters[0] = attributeValue;
				throw new FormException(ERROR_MESSAGE, new Object[] {});

			}
			else
			{
				_log.debug("LOG_SYSTEM_OUT","////Loc Valid " + attributeName + ": " + attributeValue,100L);
			}
