...
import com.ssaglobal.scm.wms.wm_invoice_processing.ui.GetTotalChargesForDisplaying;

public class ReleaseCCDisplayMessage extends FormWidgetExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ReleaseCCDisplayMessage.class);

	public ReleaseCCDisplayMessage()
	{
	}

	protected int execute(StateInterface state, RuntimeFormWidgetInterface widget) throws EpiException
	{
		 _log.debug("LOG_DEBUG_EXTENSION_RELEASE_CYCLE_COUNT","Executing ReleaseCCDisplayMessage",100L);	
		try
		{
			...
			try
			{
				args[0] = state.getServiceManager().getUserContext().get("ReleaseCCMessage").toString();
			...
			strMsg = getTextMessage("WMTXT_MESSAGE", args, locale);
			...
			state.getServiceManager().getUserContext().remove("ReleaseCCMessage");
		} catch (Exception e)
		{
			e.printStackTrace();
			return RET_CANCEL;
		}
		_log.debug("LOG_DEBUG_EXTENSION_RELEASE_CYCLE_COUNT","Exiting ReleaseCCDisplayMessage",100L);
		return 0;
	}
}
