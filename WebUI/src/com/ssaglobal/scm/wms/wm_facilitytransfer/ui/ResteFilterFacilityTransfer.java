...
import com.ssaglobal.scm.wms.uiextensions.HeaderDetailResetFilter;

public class ResteFilterFacilityTransfer extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	private static final String CONTEXT_FILTER = "HeaderDetailFilter";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(HeaderDetailResetFilter.class);
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException
	{
		...
		_log.debug("LOG_DEBUG_EXTENSION_HeaderDetailResetFilter", "NAME: " + state.getInteractionName(), SuggestedCategory.NONE);
		...
		if (form.isListForm())
		{
			...
			_log.debug("LOG_DEBUG_EXTENSION_HeaderDetailResetFilter", "Saving the Filter " + vw.toString() + ":" + viewData + " into context ", SuggestedCategory.NONE);
			return RET_CONTINUE;
		}
		else
		{
			...
			if (viewData == null)
			{
				...
				BioCollectionBean collection = (BioCollectionBean) uowb.getBioCollectionBean(new Query(bioType, null, null));
			...
			String formBioType = form.getFocus() == null ? getParameterString("LISTBIO") : form.getFocus().getDataType(); 
			String viewBioType = filter.getDataType();
			_log.debug("LOG_DEBUG_EXTENSION_HeaderDetailResetFilter", "Form Bio Type: " + formBioType + " and View Bio Type: " + viewBioType , SuggestedCategory.NONE);
			if(!formBioType.equals(viewBioType))
			{
				...
				BioCollectionBean collection = (BioCollectionBean) uowb.getBioCollectionBean(new Query(formBioType, null, null));
				result.setFocus(collection);
				return RET_CONTINUE;
			}
			...
			BioCollectionBean collection = (BioCollectionBean) uowb.getBioCollectionBean(new Query(filter.getDataType(),query_string, null));
			_log.debug("LOG_DEBUG_EXTENSION_HeaderDetailResetFilter", "Created QBEBioBean of Type:" + collection.getBioTypeName() + "..." + "Size "
