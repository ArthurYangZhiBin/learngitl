...
public class TransshipASNResetFilter extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	private String CONTEXT_FILTER = "HeaderDetailFilter";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(TransshipASNResetFilter.class);
		...
		else
		{
			...
			if(WSDefaultsUtil.isOwnerLocked(state)){
				...
				if(lockedOwners != null && lockedOwners.size() > 0){
					...
					query = "DPE('SQL','@[wm_transasn.TRANSASNKEY] in ("+innerQuery+")')";
