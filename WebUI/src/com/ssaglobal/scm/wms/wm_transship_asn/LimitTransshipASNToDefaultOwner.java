...
public class LimitTransshipASNToDefaultOwner extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitTransshipASNToDefaultOwner.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		
		_log.debug("LOG_SYSTEM_OUT","\n\n\nhere2!!!!!\n\n\n\n",100L);
		_log.debug("LOG_DEBUG_EXTENSION_LIMITSTORKEY","Executing LimitTransshipASNToDefaultOwner",100L);		
...
//			throw new UserException(errorMsg,new Object[0]);
		...
		for(int i = 0; i < lockedOwners.size(); i++){
			if(i == 0)
				lockedOwnerClause += " vTransAsn.STORERKEY = \\'" + lockedOwners.get(i) + "\\' ";
			else
				lockedOwnerClause += " OR vTransAsn.STORERKEY = \\'" + lockedOwners.get(i) + "\\' ";
			_log.debug("LOG_DEBUG_EXTENSION_LIMITSTORKEY","locked owner clause after iteration "+i+":"+lockedOwnerClause,100L);
		}				
		
		String innerQuery = " SELECT TRANSASNKEY FROM vTransAsn WHERE "+lockedOwnerClause;
									
		
		Query qry = new Query("wm_transasn","DPE('SQL','@[wm_transasn.TRANSASNKEY] in ("+innerQuery+")')","");
