...
public class UserActivityConfirmPopUp extends com.epiphany.shr.ui.view.customization.FormExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(UserActivityConfirmPopUp.class);
	...
	protected int preRenderForm(ModalUIRenderContext context, RuntimeNormalFormInterface form) throws EpiException
	{
		...
		_log.debug("LOG_DEBUG_EXTENSION_UserActivityConfirmPopUp", sourceWidget.getName(), SuggestedCategory.NONE);
		if ("COMPLTETE".equals(sourceWidget.getName()))
		{
			String assnNumber = sourceWidget.getForm().getFormWidgetByName("ASSIGNMENTNUMBER").getDisplayValue();
			if (isEmpty(assnNumber))
			{
				assnNumber = "";
			}
			String textMessage = getTextMessage("WMEXP_UA_CONFIRM", new Object[] { assnNumber }, state.getLocale());
			form.getFormWidgetByName("MESSAGE").setLabel("label", textMessage);

		}
		else if ("CREATE".equals(sourceWidget.getName()))
		{
			String iActivity = sourceWidget.getForm().getFormWidgetByName("INDIRECTACTIVITY").getDisplayValue();
			if (isEmpty(iActivity))
			{
				iActivity = "";
			}
			String textMessage = getTextMessage("WMEXP_UA_CONFIRM_INDIRECT", new Object[] { iActivity }, state.getLocale());
			form.getFormWidgetByName("MESSAGE").setLabel("label", textMessage);
		}
		return RET_CONTINUE;
	}
