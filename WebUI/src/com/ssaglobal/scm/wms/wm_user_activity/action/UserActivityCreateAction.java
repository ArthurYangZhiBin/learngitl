...
public class UserActivityCreateAction extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(UserActivityCreateAction.class);
	/**
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException
	{
		...
		if (login == true)
		{
			...
			String userAttendanceKey = activeQueryResults.getAttribValue(2).asTextData().getAsString();
			//set userAttendanceKey in session Later
			_log.debug("LOG_DEBUG_EXTENSION", "\n\t" + userAttendanceKey + "\n", SuggestedCategory.NONE);
			if (activeRecord == 0)
			{
				//Exception
			...
			String indirectActivity = (String) indirectActivityWidget.getValue();
			_log.debug("LOG_DEBUG_EXTENSION", "\n\t" + indirectActivity + "\n", SuggestedCategory.NONE);
			if (!isEmpty(indirectActivity))
			{
				//Prompts User in Modal then continues in another extension
				//set userAttendanceKey in session
				context.getState().getRequest().getSession().setAttribute("USERATTENDANCEKEY", userAttendanceKey);
				return RET_CONTINUE;
			...
			{
				//Exception
				throw new UserException("WMEXP_REQFIELD", new Object[] { removeTrailingColon(indirectActivityWidget.getLabel(
																																"label",
