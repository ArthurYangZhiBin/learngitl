...
public class UserActivityTimeInActionPostConfirm extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(UserActivityTimeInActionPostConfirm.class);
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException
	{
		...
		if ((response).equalsIgnoreCase("Yes"))
		{
			...
			//Set Login Time from Context
			Calendar ejbDate = (Calendar) context.getState().getRequest().getSession().getAttribute("UATIMEIN");
			focus.setValue("DATETIMEIN", ejbDate);
			form.setError("WMEXP_UA_CLOCK_IN", new Object[] { userID });

			//Set Logout Time
			Calendar currentDateTime = new GregorianCalendar();
			focus.setValue("DATETIMEOUT", currentDateTime);
		}
		return RET_CONTINUE;
	}
