...
public class CodesResetSequence extends
	...
	protected int execute(ActionContext context, ActionResult result)
		...
		RuntimeFormInterface parentForm = FormUtil.findForm(state
				.getCurrentRuntimeForm(), "", "wm_codes_detail_view", state);

		UnitOfWorkBean uow = state.getDefaultUnitOfWork();
		uow.clearState();

		DataBean focus = parentForm.getFocus();
		BioCollectionBean rs = (BioCollectionBean) focus
				.getValue("CODESDETAIL");
		if (rs != null) {
			try {
				for (int i = 0; i < rs.size(); i++) {
					BioBean codeDetail = rs.get("" + i);
					codeDetail.setValue("SEQUENCE", 1000);
					codeDetail.save();
				}
