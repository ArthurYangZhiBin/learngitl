...
public class SPSCarrierPreRender extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
	...
	protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
		...
		try {
						RuntimeFormWidgetInterface spsWidget = form.getFormWidgetByName("KSHIP_CARRIER");
						RuntimeFormWidgetInterface scacWidget = form.getFormWidgetByName("SCAC_CODE");

						DataBean carrierFocus = form.getFocus();
						Object spsEnabledObj = carrierFocus.getValue("KSHIP_CARRIER");
						String spsEnabled = spsEnabledObj == null?"0":spsEnabledObj.toString();
						if ("1".equalsIgnoreCase(spsEnabled)) {
						...
						context.getState().getRequest().getSession().setAttribute("SPS_ENABLED", spsEnabled);
