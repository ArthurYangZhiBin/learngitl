...
package com.ssaglobal.scm.wms.wm_allocatestrategy.ui;
...
public class AllocateStrategyPreRender extends
	...
	protected int preRenderForm(UIRenderContext context,
		...
		try {
			...
			String pickCode = BioAttributeUtil.getString(focus, "PICKCODE");
			if (StringUtils.isEmpty(pickCode)) {
				form.getFormWidgetByName("LOCALLOCATIONSORT").setProperty(
						RuntimeFormWidgetInterface.PROP_READONLY, Boolean.TRUE);
			} else {
				if ("NSPAL01_07".equals(pickCode)
						|| "NSPALSTD04".equals(pickCode)
						|| "NSPALSTD05".equals(pickCode)
						|| "NSPALSTD01".equals(pickCode)) {
					form.getFormWidgetByName("LOCALLOCATIONSORT").setProperty(
