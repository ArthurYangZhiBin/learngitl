...
public class ImportFileDelete extends ListSelector {

	private static final String STATUSQUEUED = "2";
	private static final String STATUSCOMPLETEDSUCCESSFULLY = "4";
	private static final String STATUSCOMPLETEDWITHERRORS = "5";
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ImportFileDelete.class);
		...
		if (selectedItems != null && selectedItems.size() > 0)
		{
			...
			try
			{
				...
				for (; bioBeanIter.hasNext();){
					...
					String content_id = (String)bio.get("content_id");
					String status = (String)bio.get("STATUS");
					if (status.equals(STATUSQUEUED) || 
							status.equals(STATUSCOMPLETEDSUCCESSFULLY) || 
							status.equals(STATUSCOMPLETEDWITHERRORS))
					{
					
						if (content_id!=null){
							deleteContent(content_id, uowb);
						}
						...
						throw new UserException("WMEXP_INVALID_STATUS", params);
					}
				}
//				_log.debug("LOG_SYSTEM_OUT","\n\t" + "BEFORE SAVE" + "\n",100L);
   ...
   private boolean deleteContent(String content_id, UnitOfWorkBean uowb){
	   
	   boolean result = false;
	   
	   String stmt = "wm_content.content_id = '"+content_id+"' ";
	   Query query = new Query("wm_content", stmt, "");
	   
	   
	   try{
		   
		   BioCollection bioCollection = uowb.getBioCollectionBean(query);
	   
	   
		  // _log.debug("LOG_SYSTEM_OUT","[ImportFileDelete]:deleting content_id:"+content_id+ " found:"+bioCollection.size() +" records.",100L);
		   
	   
		   if(bioCollection!=null && bioCollection.elementAt(0)!=null){
			   
			   Bio contentBio =bioCollection.elementAt(0);
			   
			   //_log.debug("LOG_SYSTEM_OUT","[ImportFileDelete]:deleting content_dataid:"+contentBio.getString("content_data_id"),100L);
			   
		
			   if(contentBio!=null){
				   Bio contentDataBio = (BioBean)contentBio.get("content_data");
				   
				   if (contentDataBio!=null)
					   contentDataBio.delete();
