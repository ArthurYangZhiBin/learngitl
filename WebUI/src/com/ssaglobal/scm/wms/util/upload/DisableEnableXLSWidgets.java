...
public class DisableEnableXLSWidgets extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
     ...
     */
	protected static ILoggerCategory _log = LoggerFactory.getInstance(DisableEnableXLSWidgets.class);
    ...
    protected int preRenderListForm(ModalUIRenderContext context, RuntimeListFormInterface form)
       ...
       try {
    	   ...
    	   String toolbarFormName = getParameterString("ToolbarForm");
    	   List widgets = (List)getParameter("Widgets");
    	   
    	   if(slotName==null || toolbarFormName == null || widgets==null){
    		   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidgets]Called with invalid parameters.",100L);
    		   return RET_CONTINUE;
    	   }
    	   ...
    	   if (slot==null){
    		   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidget]Could not find slot:"+slot.getName(),100L);
    		   return RET_CONTINUE;
    	   }else{
        	   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidget]Found target slot:"+slot.getName(),100L);
        	   
    		   RuntimeFormInterface toolbar = context.getState().getRuntimeForm(slot,toolbarFormName);
        	   
    		   if(toolbar==null){
        		   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidget]Toolbar form not found:"+toolbarFormName,100L);
        		   return RET_CONTINUE;
        	   }else{
        		   
        		   for(Iterator widgetItr = widgets.iterator(); widgetItr.hasNext();){
        			   String widgetName = (String)widgetItr.next();
        			   
        			   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidget]Looking for widget:"+widgetName,100L);
        			   
        			   RuntimeFormWidgetInterface  widget = toolbar.getFormWidgetByName(widgetName);
        			   
        			   if(widget==null){
        				   _log.debug("LOG_SYSTEM_OUT","[DisableEnableXLSWidget]Could not find widget:"+widgetName+" in Toolbar.",100L);
        				   continue;
        			   }else
