...
public class FormatTemplateId extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
     ...
     */
	protected static ILoggerCategory _log = LoggerFactory.getInstance(FormatTemplateId.class);
    ...
    protected int preRenderListForm(UIRenderContext context, RuntimeListFormInterface form)
    	...
    	_log.debug("LOG_SYSTEM_OUT","[FormatTemplateId]:Form:"+form.getName(),100L);
        try {
     	   StateInterface state = context.getState();
     	   BioCollection bioCollection = (BioCollection)form.getFocus();
            for(int i=0; i < bioCollection.size(); i++){
         	   Bio bio = (BioBean) bioCollection.elementAt(i);
         	   String templateId = (String)bio.get("TEMPLATEID");
         	   String templateidlkup = templateId.substring(0,templateId.length()-3);
         	   bio.set("TEMPLATEIDLKUP",templateidlkup);
         	   
         	   //_log.debug("LOG_SYSTEM_OUT","[FormatTemplateId]::templateid:"+templateId+"  templateidlkup:"+templateidlkup,100L);
            }
         } catch(Exception e) {
