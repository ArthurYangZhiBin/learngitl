...
public class DisableEnableToolbar extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
     ...
     */
	protected static ILoggerCategory _log = LoggerFactory.getInstance(DisableEnableToolbar.class);
    protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
       ...
       try {
    	   ...
    	   //RuntimeFormInterface toolbar = FormUtil.findForm(form, parent.getName(), "wm_upload_shell Toolbar", state);
    	   
    	   String toolbarName = getParameterString("Toolbar");
    	   //RuntimeFormInterface toolbar = FormUtil.findForm(form, form.getName(), "wm_upload_shell Toolbar", state);
    	   RuntimeFormInterface toolbar = FormUtil.findForm(form, form.getName(), toolbarName, state);
    	   
    	   _log.debug("LOG_SYSTEM_OUT","[DisableEnableToolbar]::Toolbar:"+toolbarName,100L);
    	   
    	   HttpSession session = state.getRequest().getSession();
    	   //String widgetName = (String)session.getAttribute("WIDGETNAME");
    	   //Boolean readOnly = (Boolean)session.getAttribute("READONLY");
    	   ArrayList widgetnameObject = (ArrayList)session.getAttribute("WIDGETNAME");
    	   ArrayList readonlyObject = (ArrayList)session.getAttribute("READONLY");
    	   
    	   //_log.debug("LOG_SYSTEM_OUT","widgetnameObject type:"+widgetnameObject.getClass().getName(),100L);
    	   session.removeAttribute("WIDGETNAME");
    	   session.removeAttribute("READONLY");
    	   
    	   //Default toolbar valuew
    	   //if (widgetName==null || readOnly==null){
    	   if (widgetnameObject==null || readonlyObject==null){
    		   _log.debug("LOG_SYSTEM_OUT","[DisableEnableToolbar]::disabling wigdetname:NEW, readOnly: TRUE",100L);
    		   RuntimeFormWidgetInterface newWidget = toolbar.getFormWidgetByName("NEW");
    		   if (newWidget != null)
    			   newWidget.setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.TRUE);
    		   
    		   return RET_CONTINUE;
    	   }
    	   //_log.debug("LOG_SYSTEM_OUT","[DisableEnableToolbar]::disabling wigdetname:"+widgetName+ " readOnly:"+ readOnly.toString(),100L);
    	   //toolbar.getFormWidgetByName(widgetName).setProperty(RuntimeFormWidgetInterface.PROP_READONLY, readOnly);
    	   
    	   
    	   
    	   for(int i=0; i < widgetnameObject.size(); i++ ){
    		   String widgetName = (String)widgetnameObject.get(i);
    		   String readOnly = (String)readonlyObject.get(i);
    		   _log.debug("LOG_SYSTEM_OUT","[DisableEnableToolbar]::disabling wigdetname:"+widgetName+ " readOnly:"+ readOnly.toString(),100L);
    		   
    		   if (readOnly.equalsIgnoreCase("true")){
    			   toolbar.getFormWidgetByName(widgetName).setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.TRUE);   
    		   }else if (readOnly.equalsIgnoreCase("false")){
    			   toolbar.getFormWidgetByName(widgetName).setProperty(RuntimeFormWidgetInterface.PROP_READONLY, Boolean.FALSE);   
    		   }
