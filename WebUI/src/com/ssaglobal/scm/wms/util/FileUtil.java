...
public class FileUtil
{
	...
	public static String generateFileRequest(String fileId, StateInterface state, Boolean constructReletive) throws EpiDataException{
		...
		String mimeType = "";
		String downloadAs = "";
		_log.debug("LOG_DEBUG_FILEUTIL","Executing generateFileRequest()...",100L);
		if(fileId == null || fileId.length() == 0){
			_log.debug("LOG_DEBUG_FILEUTIL","fileId invalid, exiting...",100L);
			return request;
		}
		
		UnitOfWorkBean uow = state.getDefaultUnitOfWork();
		Query bioQry = new Query("wm_fileupload","wm_fileupload.FILEUPLOADPK = '"+fileId+"'","");
		_log.debug("LOG_DEBUG_FILEUTIL","retrieving fileRecords...",100L);
		BioCollection fileRecords = uow.getBioCollectionBean(bioQry);
		if(fileRecords == null || fileRecords.size() == 0){
			_log.debug("LOG_DEBUG_FILEUTIL","fileRecords invalid, exiting...",100L);
			return request;
		}
		bioQry = new Query("content","content.content_id = '"+fileRecords.elementAt(0).get("content_id")+"'","");
		_log.debug("LOG_DEBUG_FILEUTIL","retrieving contentRecords...",100L);
		...
		_log.debug("LOG_DEBUG_FILEUTIL","mimeType:"+mimeType,100L);
