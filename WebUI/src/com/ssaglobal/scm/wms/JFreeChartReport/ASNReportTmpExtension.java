...
public class ASNReportTmpExtension extends ActionExtensionBase{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(ASNReportTmpExtension.class);
...
//Get the code from codelookup using the tooltip
			String qry1 = "codelkup.LISTNAME = 'RECSTATUS'and codelkup.DESCRIPTION = '"+ tooltip + "'";			
			...
			String qry = "receipt.STATUS ='"+ Statuscode +"'";		
			if(WSDefaultsUtil.isOwnerLocked(state)){
  	   	    	...
  	   	    	if(lockedOwners != null && lockedOwners.size() > 0){
  	   	    		qry += " AND receipt.STORERKEY IN ( '"+lockedOwners.get(0)+"' ";
  	   	    	...
  	   	    	if(lockedCarriers != null && lockedCarriers.size() > 0){
  	   	    		qry += " AND (receipt.CARRIERKEY IN ( '"+lockedCarriers.get(0)+"' ";
  	   	    		for(int i = 1; i < lockedCarriers.size(); i++){
  	   	    			qry += " , '"+lockedCarriers.get(i)+"'";
  	   	    		}
  	   	    		qry += ") OR receipt.CARRIERKEY IS NULL OR receipt.CARRIERKEY = '' OR receipt.CARRIERKEY = ' ') ";
  	   	    	}
  	   	    	ArrayList lockedVendors = WSDefaultsUtil.getLockedVendors(state);
  	   	    	if(lockedVendors != null && lockedVendors.size() > 0){
  	   	    		qry += " AND (receipt.EXTERNALRECEIPTKEY2 IN ( '"+lockedVendors.get(0)+"' ";
  	   	    		for(int i = 1; i < lockedVendors.size(); i++){
  	   	    			qry += " , '"+lockedVendors.get(i)+"'";
  	   	    		}
  	   	    		qry += " )OR receipt.EXTERNALRECEIPTKEY2 IS NULL OR receipt.EXTERNALRECEIPTKEY2 = '' OR receipt.EXTERNALRECEIPTKEY2 = ' ') ";
  	   	    	} 
			}
			Query loadBiosQry = new Query("receipt", qry, null);

			BioCollectionBean collection = (BioCollectionBean)uow.getBioCollectionBean(new Query("receipt",null,null));			
