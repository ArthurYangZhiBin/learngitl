...
public class ASNStatusDefaults extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
       ...
       try {
           ...
           StateInterface state = context.getState();
 			...
 			RuntimeFormInterface currentForm = state.getCurrentRuntimeForm();
  	   	    ...
  	   	    RuntimeFormWidgetInterface date = currentForm.getFormWidgetByName("wm_asn_sdate_param");
  	   	    Calendar temp = Calendar.getInstance();
  	   	    
  	   	    if ( session.getAttribute("_wm_asn_sdate_param") != null)
  	   	    {
  	   	    	temp = (Calendar)session.getAttribute("_wm_asn_sdate_param");
  	   	    }
  	   	    
 	   	    date.setCalendarValue(temp);

 	   	    RuntimeFormWidgetInterface num = currentForm.getFormWidgetByName("wm_asn_days_param");
 	   	    String numDays = "1";
 	   	    if ( session.getAttribute("_wm_asn_days_param")  != null)
 	   	    {
 	   	    	numDays = (String)session.getAttribute("_wm_asn_days_param");
 	   	    } 	
 	   	    num.setValue(numDays);
 	  	      	   							
 			session.setAttribute("_wm_asn_sdate_param", temp);
 			session.setAttribute("_wm_asn_days_param", numDays); 	
