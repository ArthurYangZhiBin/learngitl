...
public class POStatusDefaults extends com.epiphany.shr.ui.view.customization.FormExtensionBase {
    ...
    protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form)
       ...
       try {
			...
			RuntimeFormInterface currentForm = state.getCurrentRuntimeForm();
	   	    ...
	   	    RuntimeFormWidgetInterface closeStatusCheckBox = currentForm.getFormWidgetByName("wm_po_closestatus_param");
	   	    String closeStatusStr = "1";
	   	    if ( session.getAttribute("_wm_po_closestatus_param")  != null)
	   	    {
	   	    	closeStatusStr = (String)session.getAttribute("_wm_po_closestatus_param");
	   	    } 	
   	    	closeStatusCheckBox.setDisplayValue(closeStatusStr);
 	    							
			session.setAttribute("_wm_po_closestatus_param", closeStatusStr); 
