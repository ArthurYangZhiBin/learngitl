...
public class FileUploadDetailFormPrerenderAction extends FormExtensionBase{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(FileUploadDetailFormPrerenderAction.class);
	protected int preRenderForm(UIRenderContext context, RuntimeNormalFormInterface form) throws EpiException {
		...
		bioQry = new Query("wm_filescreenmapping","wm_filescreenmapping.FILEID = '"+fileUploadRecords.elementAt(0).get("FILEUPLOADPK")+"'","");
		BioCollection fileScreenMappings = uow.getBioCollectionBean(bioQry);
		ArrayList screens = new ArrayList(fileScreenMappings.size());
		for(int i = 0; i < fileScreenMappings.size(); i++){
			screens.add(fileScreenMappings.elementAt(i).get("SCREEN"));			
		}
		form.getFormWidgetByName("screen").setArrayValue(screens);
