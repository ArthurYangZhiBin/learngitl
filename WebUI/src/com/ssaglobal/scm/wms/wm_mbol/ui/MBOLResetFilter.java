...
public class MBOLResetFilter extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	private String CONTEXT_FILTER = "HeaderDetailFilter";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(MBOLResetFilter.class);
		...
		else
		{
			...
			if(WSDefaultsUtil.isOwnerLocked(state)){
				...
				if(lockedOwners != null && lockedOwners.size() > 0){
					...
					query = "DPE('SQL','(@[wm_mbol.MBOLKEY] IN (SELECT MBOL.MBOLKEY FROM MBOL RIGHT OUTER JOIN MBOLDETAIL ON MBOL.MBOLKEY = MBOLDETAIL.MBOLKEY WHERE MBOLDETAIL.MBOLKEY is null OR MBOLDETAIL.MBOLKEY = \\'\\' OR (((MBOLDETAIL.PALLETKEY IN (SELECT PALLETKEY FROM PALLETDETAIL WHERE  ("+lockedOwnerPalletClause+")) OR MBOLDETAIL.PALLETKEY is null OR MBOLDETAIL.PALLETKEY = \\'\\') AND (MBOLDETAIL.ORDERKEY IN (SELECT ORDERKEY FROM ORDERS WHERE  ("+lockedOwnerOrderClause+")) OR MBOLDETAIL.ORDERKEY is null OR MBOLDETAIL.ORDERKEY = \\'\\')))))')";
