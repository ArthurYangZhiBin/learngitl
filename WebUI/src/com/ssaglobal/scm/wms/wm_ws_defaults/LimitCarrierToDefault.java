...
public class LimitCarrierToDefault extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitCarrierToDefault.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		//Krishna Kuchipudi: Commented the below code for the issue SCM-00000-05555. Now Even Owner is locked, user will be able to update Carrier Record.
		/*
		...
		for(int i = 0; i < widgetNames.size(); i++){
			...
			if((ownerValue.equals(" ") || ownerValue.equals("")) && allowBlanks)
			//SM 11-28-07 SCM-00000-03677: End Update
				continue;
			if(!WSDefaultsUtil.canUserAccessCarrier(ownerValue, state)){
		...
		if(failedWidgets.length() > 0){
			...
			ArrayList lockedOwners = WSDefaultsUtil.getLockedCarriers(state);
			args[0] = "";
