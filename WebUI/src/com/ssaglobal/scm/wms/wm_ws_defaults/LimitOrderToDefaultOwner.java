...
public class LimitOrderToDefaultOwner extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitOrderToDefaultOwner.class);
...
//			throw new UserException(errorMsg,new Object[0]);
		...
		for(int i = 0; i < lockedOwners.size(); i++){
			if(i == 0)
				lockedOwnerClause += " wm_orders.STORERKEY = '" + lockedOwners.get(i) + "' ";
			else
				lockedOwnerClause += " OR wm_orders.STORERKEY = '" + lockedOwners.get(i) + "' ";
		}				
		DataBean focus = form.getFocus();		
		String orderValue = (String)focus.getValue(widgetName);
		UnitOfWorkBean uow = state.getDefaultUnitOfWork();
		Query qry = new Query("wm_orders","wm_orders.ORDERKEY = '"+orderValue+"' AND ("+lockedOwnerClause+")","");
		...
		try {
			if(orders == null || orders.size() == 0){
				...
				String errorMsg = getTextMessage("WMEXP_LIMIT_ORDER_DATA_ENTRY",args,state.getLocale());
				throw new UserException(errorMsg,new Object[0]);
			}
