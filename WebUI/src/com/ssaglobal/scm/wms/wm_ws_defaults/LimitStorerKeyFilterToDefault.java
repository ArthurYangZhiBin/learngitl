...
public class LimitStorerKeyFilterToDefault extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitStorerKeyFilterToDefault.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		String lockingOverrideQuery = (String)getParameter("lockingOverrideQuery");
		...
		String delimitedOwnerList = "";		
		ArrayList lockedOwners = WSDefaultsUtil.getLockedOwners(state);
		if(lockedOwners == null || lockedOwners.size() == 0)
			return RET_CONTINUE;
		delimitedOwnerList += "'"+lockedOwners.get(0)+"'";
		for(int i = 1; i < lockedOwners.size(); i++){
			delimitedOwnerList += ",'"+lockedOwners.get(i)+"'";
		}
		
		BioCollectionBean focus = (BioCollectionBean)result.getFocus();
		for(int i = 0; i < widgetNames.size(); i++){
			String innerQuery = focus.getBioTypeName()+"."+widgetNames.get(i)+" IN ("+delimitedOwnerList+")";	
			if(lockingOverrideQuery != null && lockingOverrideQuery.length() > 0)
				innerQuery += " OR ("+lockingOverrideQuery+")";
