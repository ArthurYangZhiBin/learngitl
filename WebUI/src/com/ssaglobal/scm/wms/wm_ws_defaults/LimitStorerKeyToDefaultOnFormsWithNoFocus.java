...
public class LimitStorerKeyToDefaultOnFormsWithNoFocus extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitStorerKeyToDefaultOnFormsWithNoFocus.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		for(int i = 0; i < widgetNames.size(); i++){
			String ownerValue = (String)form.getFormWidgetByName((String)widgetNames.get(i)).getValue();
			if(ownerValue == null)
				ownerValue = "";
			if(!WSDefaultsUtil.canUserAccessOwner(ownerValue, state)){
