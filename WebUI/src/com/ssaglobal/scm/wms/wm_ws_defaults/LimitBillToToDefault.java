...
public class LimitBillToToDefault extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitBillToToDefault.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		//Krishna Kuchipudi: Commented the below code for the issue SCM-00000-05555. Now Even Owner is locked, user will be able to update BillTo Record.
		/*
		...
		for(int i = 0; i < widgetNames.size(); i++){
			...
			//SM 11-28-07 SCM-00000-03677: End Update
				continue;
			if(!WSDefaultsUtil.canUserAccessBillTo(ownerValue, state)){
		...
		if(failedWidgets.length() > 0){
			...
			ArrayList lockedOwners = WSDefaultsUtil.getLockedBillTo(state);
			args[0] = "";
