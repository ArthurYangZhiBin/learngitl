...
public class LimitCustomerToDefault extends ActionExtensionBase{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(LimitCustomerToDefault.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		//Krishna Kuchipudi: Commented the below code for the issue SCM-00000-05555. Now Even Owner is locked, user will be able to update Customer Record.
		/*
		DataBean focus = form.getFocus();
		for(int i = 0; i < widgetNames.size(); i++){
			String customerValue = "";
			if(hasFocus){
				customerValue = (String)focus.getValue((String)widgetNames.get(i));
			}else{
				customerValue = form.getFormWidgetByName((String)widgetNames.get(i)).getDisplayValue();
				//Added because OA fix is not sending extension-set values to the back end on forms with no focus
				//Unless the user clicks the widget first. Tests have shown that if the user blanks the field then
				//NULL is sent back so in that case we do nothing and let the validation catch it.
				if(customerValue != null && customerValue.equals("")){
					customerValue = WSDefaultsUtil.getPreFilterValueByType("CUSTOM", context.getState());
					if(customerValue == null || customerValue.equals("")){
						if(WSDefaultsUtil.getLockedCustomers(state) != null && WSDefaultsUtil.getLockedCustomers(state).size() > 0){
							customerValue = (String)WSDefaultsUtil.getLockedCustomers(state).get(0);
						}
			...
			if(customerValue == null)
				customerValue = "";
			//SM 11-28-07 SCM-00000-03677: Added single space to ingored values 
			if((customerValue.equals(" ") || customerValue.equals("")) && allowBlanks)
			//SM 11-28-07 SCM-00000-03677: End Update
				continue;
			if(!WSDefaultsUtil.canUserAccessCustomer(customerValue, state)){
		...
		if(failedWidgets.length() > 0){
			...
			ArrayList lockedOwners = WSDefaultsUtil.getLockedCustomers(state);
			args[0] = "";
