...
public class InventoryBalanceComputedAttributesLot implements ComputedAttributeSupport{
		public Object get(Bio bio, String attributeName, boolean isOldValue)
			...
			if("qtyavailable".equalsIgnoreCase(attributeName)){
				...
				qtyHold = new Double(bio.get("QTYONHOLD").toString()).doubleValue();
				qty = new Double(bio.get("QTY").toString()).doubleValue();
				qtyPreAllocated = new Double(bio.get("QTYPREALLOCATED").toString()).doubleValue();
				return new Double(qty - qtyPreAllocated - qtyAllocated - qtyPicked - qtyHold);
			}
			return null;
		}
