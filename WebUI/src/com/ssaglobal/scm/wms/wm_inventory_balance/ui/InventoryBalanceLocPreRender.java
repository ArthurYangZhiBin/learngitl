...
public class InventoryBalanceLocPreRender extends FormExtensionBase{	
	
	
	protected int preRenderListForm(UIRenderContext context, RuntimeListFormInterface form) throws EpiException {		
		String locQry = (String)context.getState().getRequest().getSession().getAttribute("INV_BAL_LOC_QRY");		
		String interactionId = context.getState().getInteractionId();		
		HashMap map = new HashMap();
		map.put("INV_BAL_LOC_QRY",locQry);
		context.getState().getRequest().getSession().setAttribute(interactionId,map);
		context.getState().getRequest().getSession().removeAttribute("INV_BAL_LOC_QRY");
		return RET_CONTINUE;
	}
