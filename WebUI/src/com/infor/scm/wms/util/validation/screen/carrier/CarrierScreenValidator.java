 ...
 *******************************************************************************/
package com.infor.scm.wms.util.validation.screen.carrier;
...
public class CarrierScreenValidator extends OwnerScreenValidator{

	public CarrierScreenValidator(WMSValidationContext context) {
		super(context);
	...
	public ArrayList validate(OwnerScreenVO screen, boolean isInsert, ArrayList disableRules) throws WMSDataLayerException {
		...
		if(validateKship_carrierNotEmpty(screen)){
			if(!validateKship_carrierIsANumber(screen))
				errors.add(new Integer(RULE_KSHIP_CARRIER_MUST_BE_A_NUMBER));
			else{
				if(!validateKship_carrierZeroOrOne(screen))
					errors.add(new Integer(RULE_KSHIP_CARRIER_MUST_BE_ZERO_OR_ONE));
			}
		}
	...
	public static String getErrorMessage(int errorCode, Locale locale, OwnerScreenVO ownerScreen){
		...
		case RULE_OWNER_MUST_BE_UNIQUE:
			param = new String[2];
			param[0] = MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_SCREEN_CARRIER_FIELD_STORERKEY, locale);
			param[1] = ownerScreen.getStorerkey();
			return MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_CARRIER_SCREEN_ERROR_DUPLICATE_CARRIER, locale, param);
