 ...
 *******************************************************************************/
package com.infor.scm.wms.util.validation.screen.billto;
...
public class BilltoScreenValidator extends OwnerScreenValidator{

	public BilltoScreenValidator(WMSValidationContext context) {
		super(context);
	...
	public ArrayList validate(OwnerScreenVO screen, boolean isInsert, ArrayList disableRules) throws WMSDataLayerException {
		...
		if(doCheckFieldLength){			
			...
			if (!validateAccountingentityLengthIs64OrLess(screen)){
				errors.add(new Integer(RULE_LENGTH_ACCOUNTINGENTITY_64));
			}
		...
		if(validateParentNotEmpty(screen)) {		
			if(!validateParentDoesExist(screen.getParent(), getContext())) {
				errors.add(new Integer(RULE_PARENT_MUST_EXIST));
			}
		}
		//SRG: 9.2 Upgrade -- End
	...
	public static String getErrorMessage(int errorCode, Locale locale, OwnerScreenVO ownerScreen){
		...
		case RULE_OWNER_MUST_BE_UNIQUE:
			param = new String[2];
			param[0] = MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_SCREEN_BILLTO_FIELD_STORERKEY, locale);
			param[1] = ownerScreen.getStorerkey();
			return MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_BILLTO_SCREEN_ERROR_DUPLICATE_BILLTO, locale, param);
		...
		//SRG: 9.2 Upgrade -- Start
		case RULE_LENGTH_ACCOUNTINGENTITY_64:
			return getAttrDomErrorMessage(errorCode, locale, MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_SCREEN_OWNER_FIELD_ACCOUNTINGENTITY, locale), ownerScreen.getAccountingentity());
		...
		case RULE_PARENT_MUST_EXIST :
			return getDoesNotExistErrorMessage(errorCode, locale, MessageUtil.getResourceBundleMessage(ResourceConstants.KEY_SCREEN_OWNER_FIELD_PARENT, locale), ownerScreen.getParent());
