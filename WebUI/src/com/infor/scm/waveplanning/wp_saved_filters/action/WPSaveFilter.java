...
public class WPSaveFilter extends ActionExtensionBase{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(WPSaveFilter.class);
	protected int execute(ActionContext context, ActionResult result) throws UserException{		
		_log.debug("LOG_DEBUG_EXTENSION_SAVEFILTER","Executing WPSaveFilter",100L);			
		...
		if(filterDetailsForm != null)
			_log.debug("LOG_DEBUG_EXTENSION_SAVEFILTER","Found filterDetailsForm Form:"+filterDetailsForm.getName(),100L);			
		else
			_log.debug("LOG_DEBUG_EXTENSION_SAVEFILTER","Found filterDetailsForm Form Form:Null",100L);	
		...
		if(filterHeaderForm != null)
			_log.debug("LOG_DEBUG_EXTENSION_SAVEFILTER","Found filterHeaderForm Form:"+filterHeaderForm.getName(),100L);			
		else
			_log.debug("LOG_DEBUG_EXTENSION_SAVEFILTER","Found filterHeaderForm Form Form:Null",100L);
		
		//Enforce "One default per user" rule
		try {
			if(filterHeaderForm != null){
				boolean isItsself = false;
...
//					Query filterQry = new Query("wp_filter","wp_filter.USERDEFAULT = 'true' AND wp_filter.USERID = '"+WPUserUtil.getUserId(state)+"'","");
					...
					if(defaultFilters.size() > 0 && !isItsself){
						context.setNavigation("clickEvent1869");
						return RET_CONTINUE;
					}
				}
			}
