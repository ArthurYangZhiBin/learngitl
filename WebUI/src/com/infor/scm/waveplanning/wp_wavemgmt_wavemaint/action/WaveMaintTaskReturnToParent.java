...
public class WaveMaintTaskReturnToParent extends com.epiphany.shr.ui.action.ActionExtensionBase
{
	...
	protected int execute(ActionContext context, ActionResult result) throws EpiException
	{
		...
		if ("WAVE".equals(viewType))
		{
			...
			BioCollectionBean rs = uow.getBioCollectionBean(new Query("wm_wp_wave", "wm_wp_wave.WAVEKEY = '" + wavekey + "'", null));
			for(int i = 0; i < rs.size(); i++)
			{
				result.setFocus(rs.get("" + i));
			}
			context.setNavigation("clickEvent1505");
		}
		else
		{
			//Order Query
			String orderkey = BioAttributeUtil.getString(focus, "ORDERKEY");
			BioCollectionBean rs = uow.getBioCollectionBean(new Query("wp_orderheader", "wp_orderheader.ORDERKEY = '" + orderkey + "'", null));
			for(int i = 0; i < rs.size(); i++)
			{
				result.setFocus(rs.get("" + i));
			}
			context.setNavigation("clickEvent1512");
		}
