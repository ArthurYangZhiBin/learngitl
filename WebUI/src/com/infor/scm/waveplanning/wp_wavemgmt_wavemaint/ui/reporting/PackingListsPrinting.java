...
public class PackingListsPrinting implements ReportingInterface{
	public void doReporting(WaveReportPrintingInputObj input) throws EpiException{
		...
		HashMap paramsAndValues = null;
		Order order = null;
		ArrayList<Order> orders = input.getOrders();
		int size = orders.size();
		for(int i=0;i<size;i++){
			order = orders.get(i);
			paramsAndValues = new HashMap();
			paramsAndValues.put("p_OrdStart",order.getOrderKey());
		    paramsAndValues.put("p_OrdEnd", order.getOrderKey());
		    ...
		    String report_url= ReportUtil.retrieveReportURL(state,"CRPT46", paramsAndValues, "PackingList");			
