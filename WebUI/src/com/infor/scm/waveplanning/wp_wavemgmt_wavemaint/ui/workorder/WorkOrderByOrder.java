...
public class WorkOrderByOrder extends com.epiphany.shr.ui.action.ActionExtensionBase{
	protected static ILoggerCategory _log = LoggerFactory.getInstance(WorkOrderByOrder.class);
    		...
    		if(size != 0){
	    		...
	    		try{
		    		for(int i=0; i<size;i++){
		    			...
		    			parameters.add(new TextData(selectedItem.get("ORDERKEY").toString()+selectedItem.get("ORDERLINENUMBER").toString()));
		    			...
		    			parameters.add(new TextData(" "));
		    			parameters.add(new TextData(" "));
		    			parameters.add(new TextData(selectedItem.get("ORDERKEY").toString()));
		    			parameters.add(new TextData(selectedItem.get("ORDERLINENUMBER").toString()));
		    		...
		    		context.setNavigation("clickEvent1769");
	    		}catch(WebuiException e){
					String [] exceptionMsg = new String[1];
					exceptionMsg[0]=e.getMessage();
					throw new UserException("WPEXP_WORK_OREDER_ELIGIBLE_BY_ORDERS_FAILED", exceptionMsg);
	    		}
    		}else{//size == 0
    			context.setNavigation("clickEvent1761");
