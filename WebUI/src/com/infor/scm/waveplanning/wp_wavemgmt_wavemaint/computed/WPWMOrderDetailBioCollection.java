...
public class WPWMOrderDetailBioCollection implements ComputedAttributeSupport
{
	private static final String SORT_ORDER = "wm_orderdetail.ORDERKEY ASC, wm_orderdetail.ORDERLINENUMBER ASC";
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(WPWMOrderDetailBioCollection.class);
	...
	public Object get(Bio bio, String attributeName, boolean isOldValue) throws EpiDataException
	{
		...
		if (serviceFailed == false)
		{
			try
			{
				...
				_log.debug("LOG_INFO_EXTENSION_WPWMOrderDetailBioCollection_get", orderQuery, SuggestedCategory.NONE);
				Query orderDetailQuery = new Query("wm_orderdetail", orderQuery,
						SORT_ORDER);
