...
public class WPWMNotAllocatedOrderDetailBioCollection implements ComputedAttributeSupport
{
	private static final String SORT_ORDER = "wm_orderdetail.ORDERKEY ASC, wm_orderdetail.ORDERLINENUMBER ASC";

	protected static ILoggerCategory _log = LoggerFactory.getInstance(WPWMNotAllocatedOrderDetailBioCollection.class);
	...
	public Object get(Bio bio, String attributeName, boolean isOldValue) throws EpiDataException
	{
		...
		if (serviceFailed == false)
		{
			try
			{
				ArrayList<String> orderKeys = null;
				...
				final String orderQuery = "wm_orderdetail.ORDERKEY IN ("
						+ listOfOrderKeys
						+ ")"
						+ " AND (wm_orderdetail.OPENQTY - (wm_orderdetail.QTYALLOCATED  + wm_orderdetail.QTYPICKED ) > 0)";
				_log.debug("LOG_INFO_EXTENSION_WPWMNotAllocatedOrderDetailBioCollection_get", orderQuery, SuggestedCategory.NONE);
				Query orderDetailQuery = new Query("wm_orderdetail", orderQuery, SORT_ORDER);
