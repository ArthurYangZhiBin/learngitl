...
public class WPGraphicalFilterSendToQueryBuilder extends SaveAction{
	
	protected static ILoggerCategory _log = LoggerFactory.getInstance(WPGraphicalFilterSendToQueryBuilder.class);
	public static final String SESSION_KEY_GRAPHICAL_FILTER_RECORDS_KEY = "wp.session.key.graphical.filter.records.key";
	protected int execute(ActionContext context, ActionResult result) throws UserException{	
		...
		try {			
			for(int i = 0; i < tempTableRecords.size(); i++){
				...
				if(collectionOfEqualTypes != null && collectionOfEqualTypes.size() > 1){
					...
					if(filterType.equalsIgnoreCase("ORDERDATE")
							|| filterType.equalsIgnoreCase("DLVRDATE")
							|| filterType.equalsIgnoreCase("DLVRDATE2")
							|| filterType.equalsIgnoreCase("RSD")){
				...
				newQueryBuilderRecord.set("INTERACTIONID", state.getInteractionId());				
		...
		//state.getRequest().getSession().setAttribute(SESSION_KEY_GRAPHICAL_FILTER_RECORDS_KEY, uniqueId);
